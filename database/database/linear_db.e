...
feature -- Abstraction Function
	...
	model: REL[K, V]
		...
		do
			...
			-- You are expected to explore the available queries/commands of the REL class in MATHMODELS.
...
--		ensure
--				Current ~ create {REL [K, V]}.make_from_tuple_array (Result)
...
feature -- Implementation of Deferred Routines from `DATABASE`
	...
	search(p_key: K): detachable V
		...
		do
			...
			values.search (p_key) -- took out result to allow for compilation
	...
	insert(p_key: K; p_value: V)
		...
		do
			...
			keys.force (p_key, keys.count + 1) -- is this needed?
