...
create
	-- a list of commands that can be used as a constructor
	make

feature -- Commands

make (m: INTEGER; d: INTEGER) -- for month and day
	require -- should forbid input values that will cause invariant to fail or invalid instance to be created
		...
		valid_combination:
			...
			and
			(m = 2 implies 1 <= d and d <= 29) -- for february
	do
		-- correct implementation
		month := m -- := is used to assign, just like = in java
		day := d
	ensure
		month_set: month = m
		day_set: day = d
...
feature -- Equality
	...
	-- Current refers to current class which is BIRTHDAY
		do
...
invariant -- class invariant (characterizes what a valid BIRTHDAY object is)
	...
	valid_combination:
		...
		and
		(month = 2 implies 1 <= day and day <= 29) -- for february
