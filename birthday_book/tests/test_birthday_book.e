...
feature -- Add tests
	...
	do
		...
		add_violation_case_with_tag("names_added_to_end", agent t_postcond_add)
	end
...
--			bd: BIRTHDAY
		do
			...
			create bb.make_empty
--			create bd.make (9,14)
			bb.add ("alan", create{BIRTHDAY}.make (9, 14)) -- precond not expected because alan does not exist in empty book
	...
	t_postcond_add
		...
		do
			...
			bd.add ("alan", create {BIRTHDAY}.make(7, 2)) -- post condition violation not expected
			bd.add ("mark", create{BIRTHDAY}.make (8, 15)) -- the wrong implementation will replace bb.names[2] with [1]
		end
end
